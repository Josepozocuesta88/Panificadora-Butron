...
<div class="container py-5">
    <div class="tw-flex tw-justify-between tw-align-middle tw-ml-4">
        <div class="nav nav-tabs text-dark ">
            <h3>Productos en oferta desde htaccess 8 prueba en test</h3>
        </div>
                ...
                <div id="categorias" class="gap-3 pt-3 categorias scrollbar row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 bg-light">
                    ...
                    <div class="">
                        <div class="card h-100 rounded-3 position-relative">
                            ...
                            <i onclick="heart(this)" data-artcod="{{ $ofertaArticulo->artcod }}" class="top-0 m-2 cursor-pointer bi bi-suit-heart-fill red-heart position-absolute end-0 font-20 heartIcon"></i>
                            @else
                            <i onclick="heart(this)" data-artcod="{{ $ofertaArticulo->artcod }}" class="top-0 m-2 cursor-pointer bi bi-suit-heart position-absolute end-0 font-20 heartIcon"></i>
                            @endif
                            <figure class="m-0 overflow-hidden bg-white d-flex align-items-center justify-content-center" style="height:325px;">
                                <a href="{{ route('info', ['artcod' => $ofertaArticulo->artcod]) }}" class="d-block">
                                    ...
                                    <img src="{{ asset('images/articulos/' . $ofertaArticulo->imagenes->first()->imanom) }}" class="h-auto d-block w-100" alt="{{ $ofertaArticulo->artnom }}" title="{{ $ofertaArticulo->artnom }}" onerror="this.onerror=null; this.src='{{ asset('images/articulos/noimage.jpg') }}';">
                                    @else
                                            ...
                                            <div class="mt-3">
                                                <div class="row align-items-end ">
                                                    <button type="submit" class="btn btn-primary ms-2 col" onclick="$('#alertaStock').toast('show')">
                                                        <i class="mdi mdi-cart me-1"></i>Añadir
                                                    </button>
                                                </div>
                                            </div>
                                        </form>
                ...
                <div id="categorias"
                    ...
                    <div class="">
                        <div class="card h-100 rounded-3 position-relative">
                            ...
                            <i onclick="heart(this)" data-artcod="{{ $ofertaArticuloPer->artcod }}" class="top-0 m-2 cursor-pointer bi bi-suit-heart-fill red-heart position-absolute end-0 font-20 heartIcon"></i>
                            @else
                            <i onclick="heart(this)" data-artcod="{{ $ofertaArticuloPer->artcod }}" class="top-0 m-2 cursor-pointer bi bi-suit-heart position-absolute end-0 font-20 heartIcon"></i>
                            @endif
                            <figure
                                class="m-0 overflow-hidden bg-white d-flex align-items-center justify-content-center" style="height:325px;">
                                <a href="{{ route('info', ['artcod' => $ofertaArticuloPer->artcod]) }}" class="d-block">
                                    @if ($ofertaArticuloPer->imagenes->isNotEmpty())
                                    <img src="{{ asset('images/articulos/' . $ofertaArticuloPer->imagenes->first()->imanom) }}" class="h-auto d-block w-100" alt="{{ $ofertaArticuloPer->artnom }}" title="{{ $ofertaArticuloPer->artnom }}" onerror="this.onerror=null; this.src='{{ asset('images/articulos/noimage.jpg') }}';">
                                    @else
                                    <img src="{{ asset('images/articulos/noimage.jpg') }}" class="h-auto d-block w-100" alt="no hay imagen" title="No hay imagen">
                                    @endif
                                </a>
                            ...
                            <div class="pt-0 card-footer">
                                <ul class="list-group list-group-flush">
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div>
                                            <a class="pe-1" href="{{ route('info', ['artcod' => $ofertaArticuloPer->artcod]) }}" data-toggle="fullscreen" title="Stock disponible o no">
                                                @if ($ofertaArticuloPer->artstocon == 1 || $ofertaArticuloPer->artstock > 1)
                                                <i class="mdi mdi-archive-check font-24 text-success"></i>
                                                @else
                                                <i class="mdi mdi-archive-cancel font-24 text-danger"></i>
                                                @endif
                                            </a>
                                            <a class="pe-1" href="{{ asset('images/' . $ofertaArticuloPer->artdocaso) }}" data-toggle="fullscreen" title="Ficha técnica">
                                                <i class="uil-clipboard-alt font-24"></i>
                                            </a>
                                            <a class="pe-1" href="{{ route('info', ['artcod' => $ofertaArticuloPer->artcod]) }}" data-toggle="fullscreen" title="Información">
                                                <i class="mdi mdi-information-outline font-24"></i>
                                            </a>
                                        </div>
                                        <div class="text-end">
                                    ...
                                    <li class="list-group-item product-card">
                                        <form method="POST" action="{{ route('cart.add', ['artcod' => $ofertaArticuloPer->artcod]) }}">
                                            @csrf
                                            <div class="row">
                                                <div class="col">
                                                    @if ($ofertaArticuloPer->cajas->isNotEmpty() && config('app.caja') == 'si')
                                                    <div class="row">
                                                        <div class="quantity-input col">
                                                            <input type="number" class="quantity form-control" name="quantity" min="1" value="1">
                                                        </div>
